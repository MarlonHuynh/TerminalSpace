...
public class ShopkeeperDialogue : MonoBehaviour
{
    ...
    void Update()
    {
        if (shopKeepingCanvas.activeSelf && Input.GetKeyUp(KeyCode.Space))
        {
            if (isTyping)
            {
                finishCurrentSentence();
            }
            else if (!isTyping && waitLoop == true)
            {
                waitLoop = false; 
            }
        }
    ...
    {
        ShopkeeperText.text = pairs[count, 0];
        isTyping = false;
        waitLoop = true; 
    } 
    ...
    IEnumerator TypeText(string sentence, string expressionKey)
    {
        ...
        {
            if (ShopkeeperText.text == sentence) // when you skip with space
            {
                break; 
            }
